...
contract ProofOfHumanity is IArbitrable, IEvidence {
    ...
    constructor(
        ...
        uint arbitratorDataListLength = arbitratorDataList.length;
        ArbitratorData storage arbitratorData = arbitratorDataList[arbitratorDataListLength++];
    ...
    function addSubmissionManually(address[] calldata _submissionIDs, string[] calldata _evidence, string[] calldata _names) external onlyGovernor {
        ...
        for (uint i = 0; i < _submissionIDs.length; i++) {
            ...
            counter++;
            uint submissionRequestsLength = submission.requests.length;
            Request storage request = submission.requests[submissionRequestsLength++];
            ...
            if (bytes(_evidence[i]).length > 0)
                emit Evidence(arbitratorDataList[arbitratorDataID].arbitrator, uint160(_submissionIDs[i]), msg.sender, _evidence[i]);
        }
    ...
    function removeSubmission(address _submissionID, string calldata _evidence) external payable {
        ...
        if (bytes(_evidence).length > 0)
            emit Evidence(requestArbitrator, submission.requests.length - 1 + uint160(_submissionID), msg.sender, _evidence);
    }
    ...
    function challengeRequest(address _submissionID, Reason _reason, address _duplicateID, string calldata _evidence) external payable {
        ...
        /* solium-disable indentation */
        {
            ...
            if (currentReason != _reason) { //(uint8(_reason) - 1)
                uint256 reasonBit = 1 << (uint256(_reason)); // Get the bit that corresponds with reason's index.
                require((reasonBit & ~request.usedReasons) == reasonBit, "The reason has already been used");

                request.usedReasons ^= uint8(reasonBit); // Mark the bit corresponding with reason's index as 'true', to indicate that the reason was used.
        ...
        emit Dispute(
            ...
            submission.requests.length - 1 + uint160(_submissionID)
        );

        if (bytes(_evidence).length > 0)
            emit Evidence(arbitratorData.arbitrator, submission.requests.length - 1 + uint160(_submissionID), msg.sender, _evidence);
    }
     ...
     */
    function executeRequest(address payable _submissionID) external {
        Submission storage submission = submissions[_submissionID];
     ...
     *  @return status submissionTime index registered hasVouched numberOfRequests
     *  The information of the submission.
     */
     ...
     *  @param _requestID The request to query.
     *  @param _challengeID The challenge to query.
     *  @return lastRoundID challenger disputeID ruling duplicateSubmissionIndex The information of the challenge.
     */
     ...
     *  @return disputed resolved requesterLost currentReason nbParallelDisputes lastChallengeID arbitratorDataID 
     *  requester ultimateChallenger usedReasons The request information.
     */
     ...
     *  @param _challengeID The challenge to query.
     *  @param _round The round to query.
     *  @return appealed paidFees sideFunded  The round information.
     */
